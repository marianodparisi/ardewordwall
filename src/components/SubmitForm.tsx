...
export function SubmitForm({ sessionId, maxChars, allowName, allowEmoji }: SubmitFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const ok = await submit(word, showName ? authorName : undefined)
    if (ok) {
      setWord('')
      setAuthorName('')
      setShowName(false)
    ...
    <form onSubmit={handleSubmit} className="mt-6">
      ...
      <div className="mt-3 flex items-center justify-between">
        <p className="text-xs text-gray-400">
          Max {maxChars} caracteres. Timer de 5 min entre envios.
        </p>
        <Link
          to="/"
